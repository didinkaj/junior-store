...
if($time == ""){
		...
		$selected_event = '<option value="personal seee" class="not">personal seee</option>';
	...
	if($date <= $today){
		...
		$selected_event = '<option value="Burial" class="not">Burial</option>';
		date_default_timezone_set('Asia/Manila');
		$datevalue = "";
	...
	if($numDate >= 1){
		...
		echo "<script>window.location='reservationB.php?target=$getID&event=$event&date=$getdate&time=$time'</script>";
	}}}
?>