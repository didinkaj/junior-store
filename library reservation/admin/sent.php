...
?>
<?php
		$queryAdminUnread = mysql_query("SELECT * FROM msg WHERE sender = 'Administrator' AND define = 'Administrator' ORDER BY id DESC");
		$numSent = mysql_num_rows($queryAdminUnread);
?>
<html>
<head>
	<title>Sent Messages (<?php echo $numSent ?>) | UOE Main Library, Main compus, eldoret</title>
...
<?php
	$queryInbox = mysql_query("SELECT * FROM msg WHERE sender = 'Administrator' AND define = 'Administrator'");
	$numQuerySent = mysql_num_rows($queryInbox);
?>
<div id="contents">
	<div id="main-content">
	<br /><br />
	<center><h1 class="title">Sent Messages (<?php echo $numQuerySent ?>)</h1></center>
