...
<div id="sublink">
	...
	<a href="reserved.php" class="subLink">Reserved Events (<b><?php echo $numReserved ?>)</a></b> |
	<b><a href="#" class="subLink">Pending Events (<?php echo $numPending ?>)</a></b> |
...
<?php
	$qNum = mysql_query("SELECT * FROM reserved WHERE status = 'pending'");
...
<option value="Wedding"> seat 1 </option>
<option value="Christening"> Seat 2 </option>
<option value="Burial"> Seat 3 </option>
<?php
...
if(isset($_POST['srch_keyword'])){
	...
	$filter_status = "AND status = 'pending'";
	...
	$query = mysql_query("SELECT * FROM reserved WHERE status = 'pending' $insert_keyword "." $filterdb LIMIT $start, $perpage"); 
}else{
	$query = mysql_query("SELECT * FROM reserved WHERE status = 'pending' $filterdb LIMIT $start, $perpage"); 
}
...
while($row = mysql_fetch_array($query)){ 
	...
	$query_expired = mysql_query("SELECT * FROM reserved WHERE status='pending' AND datelimit <='$today'");
	$row_expired =  mysql_num_rows($query_expired);
	
	if($row_expired > 0){
	$array = mysql_fetch_array($query_expired);
	$query_delete = mysql_query("DELETE FROM reserved WHERE status='pending' AND datelimit <='$today'");
	if($query_delete){
		echo "<script>Alert.render('".$row_expired." Pending Reservation has been deleted because of no reponse by the Administrator<br/>3 days after the Date Registered.')</script>";
		echo "<script>setInterval( function(){ window.location='pending.php' }, 9000 )</script>";
	}else{
		echo "<script>Alert.render('Error Occur!')</script>";
		$db_error = mysql_error();
		$url = $_SERVER['REQUEST_URI'];
		$error_log =  mysql_query("INSERT INTO errorlog (error, url) VALUES ('$db_error', '$url')");
	}
	}
...
<tr>
<td>
<a href="viewPending.php?target=<?php echo $id1 ?>" style="color: #000; cursor: default;" title="Click for details">
...
function deletePost(id){
	document.location.href="functions/delpending.php?target=" + id;
}
</script>
<?php 
echo "<script>";
